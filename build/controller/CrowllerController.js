...
var checkLogin = function (req, res, next) {
    function isLoginFn(req) {
        return !!(req.session ? req.session.login : undefined);
    }
    var isLogin = isLoginFn(req);
    if (isLogin) {
        next();
    }
    else {
...
var CrowllerController = /** @class */ (function () {
    ...
    CrowllerController.prototype.getData = function (req, res) {
        ...
        var dellAnalyzer = analyzer_1.default.getInstance();
    ...
    CrowllerController.prototype.showData = function (req, res) {
        try {
            ...
            var data = fs_1.default.readFileSync(position, "utf-8");
            res.json(util_1.getResponseData(JSON.parse(data)));
        }
        catch (err) {
